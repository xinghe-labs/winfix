...
- Added `baseline` and `compare` modes: snapshot drives, startup items, auto-start-but-stopped services, and pending-reboot state into `~\.winfixaselines\` (newest 20 kept), then diff any later moment against it (per-drive byte deltas, startup added/removed, service count change).
